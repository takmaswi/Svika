import Image from "next/image";

import ThemeToggle from "@/components/passenger/ThemeToggle";

interface LandingHeroProps {
  /** Public asset path to the hero image. Falls back to a brand gradient when missing. */
  imageSrc: string;
  hasImage: boolean;
}

/**
 * Brand landing hero — full-bleed inspiration image with a glass overlay
 * carrying the headline and subline. Phase 3.7 visual rebuild; R5 swaps the
 * teal gradient fallback + tints to the new Apple-blue token and exposes the
 * ThemeToggle icon variant in the top-right of the hero header.
 */
export default function LandingHero({ imageSrc, hasImage }: LandingHeroProps) {
  return (
    <section
      className="relative isolate w-full overflow-hidden"
      aria-labelledby="svika-hero-headline"
      style={{ minHeight: "62vh" }}
    >
      <div className="absolute inset-0 svika-animate-image-fade">
        {hasImage ? (
          <Image
            src={imageSrc}
            alt=""
            fill
            priority
            sizes="100vw"
            style={{ objectFit: "cover" }}
          />
        ) : (
          <div
            className="h-full w-full"
            style={{
              background:
                "linear-gradient(135deg, var(--color-action) 0%, var(--color-action-hover) 100%)",
            }}
          />
        )}
        <div
          aria-hidden
          className="absolute inset-0"
          style={{
            background:
              "linear-gradient(180deg, rgba(255,255,255,0) 35%, rgba(255,255,255,0.45) 70%, rgba(255,255,255,0.85) 100%)",
          }}
        />
      </div>

      <div className="relative z-10 flex h-full min-h-[62vh] flex-col px-5 pb-8 pt-8">
        <header className="flex items-baseline justify-between gap-3">
          <h1
            className="svika-display"
            style={{ lineHeight: 1, color: "var(--color-ink)" }}
          >
            Svika
          </h1>
          <div className="flex items-center gap-3">
            <p
              className="svika-meta uppercase"
              style={{ color: "var(--color-ink-mute)" }}
            >
              the kombi network in your pocket
            </p>
            <ThemeToggle variant="icon" />
          </div>
        </header>

        <div className="mt-auto svika-animate-glass-rise">
          <div className="svika-glass-strong px-5 py-5">
            <h2
              id="svika-hero-headline"
              className="svika-display"
              style={{ color: "var(--color-ink)" }}
            >
              Where every kombi is one tap away
            </h2>
            <p
              className="svika-body mt-2"
              style={{ color: "var(--color-ink-mute)" }}
            >
              Plan it. Pay it. Board it. Send it. Same kombi.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}
